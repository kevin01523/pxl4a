...
extern void
	susfs_auto_add_sus_ksu_default_mount(const char __user *to_pathname);
...
bypass_orig_flow:
	...
	if (susfs_is_current_zygote_domain()) {
		...
		if (mnt_ns) {
			get_mnt_ns(mnt_ns);
			rcu_read_lock();
			mnt_id = list_first_entry(&mnt_ns->list, struct mount,
						  mnt_list)
					 ->mnt_id;
		...
		struct user_namespace *user_ns, struct fs_struct *new_fs)
{
	...
	if (is_zygote_pid) {
		last_entry_mnt_id =
			list_first_entry(&new_ns->list, struct mount, mnt_list)
				->mnt_id;
